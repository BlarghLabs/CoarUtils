...
namespace CoarUtils.commands.ipgeolocation.maxmind {
  ...
  /// alongside the near-identical ipgeolocation.GetGeolocation.
  ///
  /// client.City() is a NETWORK CALL to MaxMind and was synchronous, blocking a request thread for
  /// the round trip; it is CityAsync now. The generic catch also used a bare "unexpected error"
  /// literal rather than Constants.ErrorMessages.UNEXPECTED_ERROR_STATUS - the same string, but the
  /// literal drifts and the constant does not.
  /// </summary>
  public static class GetGeolocationViaService {
    ...
    public static async Task<Response> Execute(
      ...
      try {
        ...
        // The account key is a credential and must never reach a log. The old spelling of this was
        // "DO_LOG_LOG", which reads as the opposite of what it does.
        if (request != null) {
